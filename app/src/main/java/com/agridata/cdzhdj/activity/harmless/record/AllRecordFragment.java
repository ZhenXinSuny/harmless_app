 ...
 * @Description :描述
 */
public class AllRecordFragment extends BaseFragment<FragmentYiRecordBinding> {
    ...
    public static AllRecordFragment newInstance(String startTime, String endTime) {
        AllRecordFragment fragment = new AllRecordFragment();
    ...
    private void getNetApply() {
        showLoading("网络数据获取中...");
        HttpRequest.getCollectedInfo(requireActivity(), startTime + " " + "00:00:00", endTime + " " + "23:59:59", -1, RoleSP.getInstance().getRoleInfo().harmlessUser.factory.Mid, RoleSP.getInstance().getRoleInfo().harmlessUser.userId, new CallBackLis<>() {
            @Override
