...
import com.agridata.cdzhdj.databinding.ActivityAddVeterinaryDrugBinding;
 ...
 * @Date : on 2023-05-05 13:33.
 * @Description :描述
 */
public class AddVeterinaryDrugActivity  extends BaseActivity<ActivityAddVeterinaryDrugBinding> implements View.OnClickListener {
    private final static String TAG = "AddVeterinaryDrugActivity------》";
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, AddVeterinaryDrugActivity.class);
    ...
    @Override
    protected ActivityAddVeterinaryDrugBinding getBinding() {
        return ActivityAddVeterinaryDrugBinding.inflate(getLayoutInflater());
    }
    ...
    protected void initView() {
        ...
        addFeedStorageBean.GoodsType.Name = "兽药";
        addFeedStorageBean.GoodsType.key = "6101";
    ...
    public void onClick(View v) {
        if (v==binding.xdrNameEt){
            ...
            if (xdrBeanList.size()>0){
                ...
                Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this,"该手机号当前暂无备案"));
            }
    ...
    private void  submitData(){
        ...
        showLoading("正在提交中...");
        HttpRequest.addFeed(AddVeterinaryDrugActivity.this,addFeedStorageBean, new CallBackLis<StatusData>() {
            @Override
            public void onSuccess(String method, StatusData content) {
                LogUtil.d("lzx---------》", content.toString());
                hideLoading();
                if (content.ErrorCode == 0) {
                    Objects.requireNonNull(RxToast.success(AddVeterinaryDrugActivity.this,"兽药入库提交成功"));
                    finish();
                }else {
                    Objects.requireNonNull(RxToast.error(AddVeterinaryDrugActivity.this,content.Message));
                }
            }

            @Override
            public void onFailure(String method, String error) {
                Objects.requireNonNull(RxToast.error(AddVeterinaryDrugActivity.this,error));
            }
        });
    ...
    private boolean checkInfo() {
        if (TextUtils.isEmpty(binding.feedManufacturerEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入兽药厂商"));
            return false;
        }

        if (TextUtils.isEmpty(binding.feedBusinessEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入兽药经营企业"));
            return false;
        }
        if (TextUtils.isEmpty(binding.feedBrandEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入兽药名称"));
            return false;
        }
        if (TextUtils.isEmpty(binding.feedBatchEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入批次"));
            return false;
        }
        if (TextUtils.isEmpty(binding.dateProductionEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请选择生产日期"));
            return false;
        }
        if (TextUtils.isEmpty(binding.validityEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入有效期"));
            return false;
        }
        if (TextUtils.isEmpty(binding.storageQuantityEt.getText().toString())) {
            Objects.requireNonNull(RxToast.warning(AddVeterinaryDrugActivity.this, "请输入入库数量(克/毫升)"));
            return false;
        }
    ...
    private  void  getXdrForTel(){
        HttpRequest.getMobileXdr(AddVeterinaryDrugActivity.this, UserDataSP.getInstance().getUserInfo().Result.mobile, new CallBackLis<XdrFeedData>() {
            @Override
    ...
    private void initLoadingView() {
        this.mLoadingDialog = new CustomLoadingDialog(AddVeterinaryDrugActivity.this);
