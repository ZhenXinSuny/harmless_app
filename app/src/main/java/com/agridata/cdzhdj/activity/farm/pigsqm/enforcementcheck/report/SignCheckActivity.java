 ...
 * @Description :描述
 */
public class SignCheckActivity extends BaseActivity<ActivitySignBinding> {

    private String name;
    private String filePath;

    public static void start(Activity context, String name) {
        Intent intent = new Intent(context, SignCheckActivity.class);
    ...
    protected void initView() {
        ...
        binding.bannerRightText.setOnClickListener(v -> {
            ...
            try {
                ...
                e.printStackTrace();
                ToastUtil.showToast(SignCheckActivity.this, "保存签名失败");
                return;
            }
            switch (name){
                case "主办人员签名":
                    RxBus.getInstance().post("ZBRYQM", filePath);
                    break;
                case "被检查单位负责人签名":
                    RxBus.getInstance().post("DWFZRQM", filePath);
                    break;
                case "见证人签名":
                    RxBus.getInstance().post("JZRQM", filePath);
                    break;
                default:
                    break;
