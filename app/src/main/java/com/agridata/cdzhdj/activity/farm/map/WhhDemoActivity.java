 ...
 * @Date : on 2023-08-06 16:36.
 * @Description :描述
 */
public class WhhDemoActivity extends BaseActivity<ActivityBaseWebviewTitleBinding> {
    ...
    public static void start(Context context, int menuId) {
        Intent intent = new Intent(context, WhhDemoActivity.class);
        Bundle data = new Bundle();
        data.putSerializable("menuId", menuId);
    ...
    private void getArguments() {
        ...
        if (menuId==1) {
            binding.titlebarMiddle.setText("无害化保险公司管理");
            url = UrlUtils.WHHBXGS;
        }else  if (menuId==2){
            binding.titlebarMiddle.setText("保险公司新增");
            url = UrlUtils.WHHBXGSXZ;
        } else if (menuId==3){
            binding.titlebarMiddle.setText("无害化保险公司人员管理");
            url = UrlUtils.WHHBXGSADD;
        } else if (menuId==4){
            binding.titlebarMiddle.setText("人员新增");
            url = UrlUtils.WHHBXGSRUXZ;
        }else if (menuId==5){
            binding.titlebarMiddle.setText("无害化已投保收集单管理");
            url = UrlUtils.WHHTBSJDGL;
        }else if (menuId==6){
            binding.titlebarMiddle.setText("无害化已投保收集单推送");
            url = UrlUtils.WHHTBSJDTS;
        }else  if (menuId==7){
            binding.titlebarMiddle.setText("屠宰场无害化申报管理");
            url = UrlUtils.TZCWHHSB;
        }
