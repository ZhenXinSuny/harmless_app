 ...
 * @time 14:04.
 */
public class XianChangShenHeWebViewActivity extends BaseActivity<ActivityWebviewBinding> {
    private final static String TAG = "lzx------》";
    private Handler mHandler = new Handler();
    private WebSettings webSettings;
    private static final String url = UrlUtils.XCSH_URL;


    public static void start(Context context) {
        Intent intent = new Intent(context, XianChangShenHeWebViewActivity.class);
...
//        binding.webView.addJavascriptInterface(this,"nativeMethod");
    ...
    public void callVueJSUserInfo() {
        binding.webView.post(new Runnable() {
            ...
            public void run() {
                LogUtil.d(TAG, "调用js的方法");
                LoginData.ResultBean result = UserDataSP.getInstance().getUserInfo().Result;
                String s = GsonUtil.toJson(result);
                binding.webView.loadUrl("javascript:callVueJSUserInfo('" + s + "')");
    ...
    private class JavaScriptCallbackThisJavaMethod {
        ...
        public void toActivity(String activityName) {
            ...
            CollectFillInActivity.start(XianChangShenHeWebViewActivity.this, activityName);
        ...
        public void toSignActivity(String activityName) {
            //此处应该定义常量对应，同时提供给web页面编写者
            LogUtil.d(TAG, activityName);
            JianDuYuanSignActivity.start(XianChangShenHeWebViewActivity.this, activityName, 1004);
        }
