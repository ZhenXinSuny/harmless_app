package com.agridata.cdzhdj.activity.harmless.CanYuWu;
 ...
 * @time 10:54.
 */
public class CanYuWuWebViewActivity extends BaseActivity<ActivityWebviewBinding> {
    private final static String TAG = "lzx------》";
    private Handler mHandler = new Handler();
    private WebSettings webSettings;
    private static  final  String url= UrlUtils.CANYUWU_URL;


    public static void start(Context context) {
        Intent intent = new Intent(context, CanYuWuWebViewActivity.class);
...
//        binding.webView.addJavascriptInterface(this,"nativeMethod");
        ...
        webSettings.setAllowFileAccess(false);    // 可以读取文件缓存
...
//        webSettings.setCacheMode(WebSettings.LOAD_CACHE_ELSE_NETWORK);//没有网络时加载缓存
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        switch (requestCode){
            ...
            case 1004:
                ...
                break;
            case 1005:
                LogUtil.d(TAG,"step------------->2");
                if (!TextUtils.isEmpty(data.getStringExtra("picMid"))){
                    String picMid = data.getStringExtra("picMid");
                    if (!picMid.equals("1")){
                        callVueJSPicShenHeYuan(data.getStringExtra("picMid"));
                        LogUtil.d(TAG,"回调的签名照片"+ data.getStringExtra("picMid"));
                    }
                }
                break;
            case 1006:
                LogUtil.d(TAG,"step------------->3");
                if (!TextUtils.isEmpty(data.getStringExtra("picMid"))){
                    String picMid = data.getStringExtra("picMid");
                    if (!picMid.equals("1")){
                        callVueJSPicCanYuWu(data.getStringExtra("picMid"));
                        LogUtil.d(TAG,"回调的签名照片"+ data.getStringExtra("picMid"));
                    }
                }
                break;
    ...
    public void callVueJSPicShenHeYuan(String id) {
        binding.webView.post(new Runnable() {
            @Override
            public void run() {
                LogUtil.d(TAG,"调用js的方法");
                binding.webView.loadUrl("javascript:callJsFunctionShenHeRen('"+id+"')");
            }
        });
    }
    public void callVueJSPicCanYuWu(String id) {
        binding.webView.post(new Runnable() {
            @Override
            public void run() {
                LogUtil.d(TAG,"调用js的方法");
                binding.webView.loadUrl("javascript:callJsFunctionCanYuWu('"+id+"')");
            }
        });
    }
    ...
    private class JavaScriptCallbackThisJavaMethod {
        ...
        public void toActivity(String activityName) {
            ...
            CollectFillInActivity.start(CanYuWuWebViewActivity.this,activityName);
        ...
        public void toSignActivity(String activityName) {
            //此处应该定义常量对应，同时提供给web页面编写者
            LogUtil.d(TAG,activityName);
            JianDuYuanSignActivity.start(CanYuWuWebViewActivity.this,activityName,1004);
        }

        @JavascriptInterface
        public void toSignShenHeRenActivity(String activityName) {
            //此处应该定义常量对应，同时提供给web页面编写者
            LogUtil.d(TAG,activityName);
            JianDuYuanSignActivity.start(CanYuWuWebViewActivity.this,activityName,1005);
        }
        @JavascriptInterface
        public void toSignCanYuWuActivity(String activityName) {
            //此处应该定义常量对应，同时提供给web页面编写者
            LogUtil.d(TAG,activityName);
            JianDuYuanSignActivity.start(CanYuWuWebViewActivity.this,activityName,1006);
        }
