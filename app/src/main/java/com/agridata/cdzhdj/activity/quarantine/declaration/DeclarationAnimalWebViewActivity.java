package com.agridata.cdzhdj.activity.quarantine.declaration;
...
import com.agridata.cdzhdj.databinding.ActivityDeclarationAnimalWebviewBinding;
 ...
 * @Description :描述 检疫流程运行页面嵌套html
 */
public class DeclarationAnimalWebViewActivity extends BaseActivity<ActivityDeclarationAnimalWebviewBinding> implements BlueManager.BlueManagerCallback {
    private final static String TAG = "lzx------》";
    private WebSettings webSettings;
    private static final String url = UrlUtils.SBSL;
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, DeclarationAnimalWebViewActivity.class);
        context.startActivity(intent);
    }

    @Override
    protected ActivityDeclarationAnimalWebviewBinding getBinding() {
        return ActivityDeclarationAnimalWebviewBinding.inflate(getLayoutInflater());
    }
    ...
    private void initBluetooth() {
        ...
        if (mBluetooth == null) {
            Objects.requireNonNull(RxToast.warning(DeclarationAnimalWebViewActivity.this, "本机未找到蓝牙功能"));
        }
    }
...
webSettings.setAppCacheEnabled(false);    //开启H5(APPCache)缓存功能
    ...
    private class JavaScriptCallbackThisJavaMethod {
        ...
        public void toPicActivity(String type) {
            //此处应该定义常量对应，同时提供给web页面编写者
            LogUtil.d(TAG, type + "");
            PicUtils.showPicDialog(DeclarationAnimalWebViewActivity.this, type);
    ...
    public void onManagerDeviceConnected(BlueDevice blueDevice) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Objects.requireNonNull(RxToast.success(DeclarationAnimalWebViewActivity.this, "连接成功..."));
    ...
    public void onManagerDeviceDisconnected(BlueDevice blueDevice) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                Objects.requireNonNull(RxToast.success(DeclarationAnimalWebViewActivity.this, "断开连接成功..."));
            }
        });
    }
...
//                        break;
        ...
        XXPermissions.with(this)
                ...
                .request(new OnPermissionCallback() {
                    ...
                    public void onGranted(List<String> permissions, boolean all) {
                        if (all) {
                            Objects.requireNonNull(RxToast.info(DeclarationAnimalWebViewActivity.this, "权限获取成功"));
                            if (!mBluetooth.isEnabled()) {
                                ...
                                if (!TextUtils.isEmpty(tagReaderBle)) { //如果连接过一次 下次就自动连接
                                    ...
                                    Objects.requireNonNull(RxToast.success(DeclarationAnimalWebViewActivity.this, "开始搜索蓝牙设备..."));
                                    mBlueManager.scanForDevice(BlueManager.BFM | BlueManager.LET);
                                }
                            }
                            ...
                            Objects.requireNonNull(RxToast.warning(DeclarationAnimalWebViewActivity.this, "获取部分权限成功，但部分权限未正常授予"));
                        }
                    }
                    ...
                    public void onDenied(List<String> permissions, boolean never) {
                        if (never) {
                            Objects.requireNonNull(RxToast.error(DeclarationAnimalWebViewActivity.this, "被永久拒绝授权，请手动开启"));
                            // 如果是被永久拒绝就跳转到应用权限系统设置页面
                            XXPermissions.startPermissionActivity(DeclarationAnimalWebViewActivity.this, permissions);
                        } else {
                            Objects.requireNonNull(RxToast.error(DeclarationAnimalWebViewActivity.this, "获取权限失败"));
                        }
                    }
                });
    }
    ...
    protected void scanBluetooth() {
        Intent intent = new Intent(DeclarationAnimalWebViewActivity.this, ConnBlueToothActivity.class);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MESSAGE_EXCEPTION_RECV:
                    if (!isFinishing()) {
                        Objects.requireNonNull(RxToast.error(DeclarationAnimalWebViewActivity.this, "断开成功..."));
                        binding.bleLayout.setVisibility(View.GONE);
                        disconnect();
                    }
                    break;
                case MESSAGE_CONNECT_LOST:
                    hideLoading();
                    Objects.requireNonNull(RxToast.error(DeclarationAnimalWebViewActivity.this, "蓝牙连接失败，请检查设备是否打开并进行重新连接"));
                    disconnect();
                    mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            Intent intent1 = new Intent(DeclarationAnimalWebViewActivity.this, ConnBlueToothActivity.class);
                ...
                case MESSAGE_RECV:
                    ...
                    if (TextUtils.isEmpty(binaryCode)) {
                        Objects.requireNonNull(RxToast.error(DeclarationAnimalWebViewActivity.this, "请扫描指定耳标"));
                    } else {
...
/**************************************  上传图片 ****************************************/
    ...
    private void upLoadImg(String filePath, String type) {
        showLoading("图片上传中...");
        HttpRequest.upLoadImg(DeclarationAnimalWebViewActivity.this, filePath, new CallBackLis<ImgBean>() {
            @Override
            public void onSuccess(String method, ImgBean content) {
                LogUtil.d(TAG, "上传图片" + content.toString());
                if (content.Status == 0) {
                    hideLoading();
                    Objects.requireNonNull(RxToast.success(DeclarationAnimalWebViewActivity.this, "上传成功~"));
