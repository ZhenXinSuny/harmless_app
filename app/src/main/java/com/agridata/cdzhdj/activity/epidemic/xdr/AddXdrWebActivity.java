 ...
 * @time 14:04.
 */
public class AddXdrWebActivity extends BaseActivity<ActivityAddXdrWebBinding> {
    private final static String TAG = "lzx------》";
    private Handler mHandler = new Handler();
    private WebSettings webSettings;
    private static  final  String url= UrlUtils.ADD_XDR_URL;
    private ImageEngine imageEngine;
    private String mTypeName;
    public static void start(Context context) {
        Intent intent = new Intent(context, AddXdrWebActivity.class);
...
binding.webView.evaluateJavascript("window.localStorage.setItem('" + key4 + "','" + UserDataSP.getInstance().getUserInfo().Result.token + "');",null);
    ...
    private void TakePicturesAlone() {
        // Take pictures alone
        PictureSelectionCameraModel cameraModel = PictureSelector.create(AddXdrWebActivity.this)
    ...
    private void OpenPicture() {
        // 进入相册
        PictureSelectionModel selectionModel = PictureSelector.create(AddXdrWebActivity.this)
    ...
    private void upLoadImg(String filePath) {
        HttpRequest.upLoadImg(AddXdrWebActivity.this, filePath, new CallBackLis<ImgBean>() {
            @Override
            public void onSuccess(String method, ImgBean content) {
                LogUtil.d(TAG, "上传图片" + content.toString());
                if (content.Status == 0) {
                    ToastUtil.showToast(AddXdrWebActivity.this, "上传成功~");
