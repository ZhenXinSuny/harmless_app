package com.agridata.cdzhdj.activity.harmless.dianhuashenbao;
 ...
 * @time 14:04.
 */
public class DianHuaShenBaoWebViewActivity extends BaseActivity<ActivityWebviewBinding> {
    private final static String TAG = "lzx------》";
    private Handler mHandler = new Handler();
    private WebSettings webSettings;
    private static  final  String url= UrlUtils.DIANHUA_SHENBAOYUAN;
    private ImageEngine imageEngine;
    private  int PicBtnType  = 0;

    public static void start(Context context) {
        Intent intent = new Intent(context, DianHuaShenBaoWebViewActivity.class);
...
webSettings.setAppCacheEnabled(false);    //开启H5(APPCache)缓存功能
    ...
    private class JavaScriptCallbackThisJavaMethod {
        ...
        public void toActivity(String activityName) {
            ...
            CollectFillInActivity.start(DianHuaShenBaoWebViewActivity.this,activityName);
        ...
        public void toBianJiActivity(String activityName) {
            //此处应该定义常量对应，同时提供给web页面编写者
            CollectEditFillInActivity.start(DianHuaShenBaoWebViewActivity.this,activityName);
        ...
        public void toSignActivity(String activityName) {
            ...
            JianDuYuanSignActivity.start(DianHuaShenBaoWebViewActivity.this,activityName,1004);
        }
    ...
    private void TakePicturesAlone() {
        // Take pictures alone
        PictureSelectionCameraModel cameraModel = PictureSelector.create(DianHuaShenBaoWebViewActivity.this)
    ...
    private void OpenPicture() {
        // 进入相册
        PictureSelectionModel selectionModel = PictureSelector.create(DianHuaShenBaoWebViewActivity.this)
    ...
    private void upLoadImg(String filePath) {
        HttpRequest.upLoadImg(DianHuaShenBaoWebViewActivity.this, filePath, new CallBackLis<ImgBean>() {
            @Override
            public void onSuccess(String method, ImgBean content) {
                LogUtil.d(TAG, "上传图片" + content.toString());
                if (content.Status == 0) {
                    ToastUtil.showToast(DianHuaShenBaoWebViewActivity.this, "上传成功~");
                }
                if (PicBtnType == 1) {
                    LogUtil.d(TAG, "身份证号Mid" +  content.Result);
                    callVueJSPicPaiZhao(content.Result);
                } else if (PicBtnType == 2) {
                    LogUtil.d(TAG, "银行卡号Mid" +  content.Result);
