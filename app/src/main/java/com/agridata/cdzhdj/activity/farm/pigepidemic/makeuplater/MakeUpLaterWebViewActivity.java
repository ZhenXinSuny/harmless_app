 ...
 * @Date : on 2023-06-16 15:58.
 * @Description :描述
 */
public class MakeUpLaterWebViewActivity extends BaseActivity<ActivityBaseWebviewTitleBinding> {
    ...
    public static void start(Context context, int menuId) {
        Intent intent = new Intent(context, MakeUpLaterWebViewActivity.class);
    ...
    private void getArguments() {
        ...
        if (menuId==1) {
            binding.titlebarMiddle.setText("先打后补申请");
            url = UrlUtils.XDHBSB;
        }else  if (menuId==2) {
            binding.titlebarMiddle.setText("疫苗采购");
            url = UrlUtils.YMCG;
        }else  if (menuId==3) {
            binding.titlebarMiddle.setText("疫苗销售");
            url = UrlUtils.YMXS;
        }else  if (menuId==4) {
            binding.titlebarMiddle.setText("销售台账");
            url = UrlUtils.XIAOSHOUTAIZHANG;

        }else  if (menuId==5) {
            binding.titlebarMiddle.setText("防疫信息填报");
            url = UrlUtils.FYXXTB;
        }else  if (menuId==6) {
            binding.titlebarMiddle.setText("免疫效果评价上传");
            url = UrlUtils.PINGJIA;

        }else  if (menuId==7) {
            binding.titlebarMiddle.setText("补助申请");
            url = UrlUtils.BUZHUSHENQING;
