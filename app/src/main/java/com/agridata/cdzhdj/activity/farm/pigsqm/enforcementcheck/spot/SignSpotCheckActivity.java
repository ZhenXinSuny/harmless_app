 ...
 * @Description :描述
 */
public class SignSpotCheckActivity extends BaseActivity<ActivitySignBinding> {

    private String name;
    private String filePath;

    public static void start(Activity context, String name) {
        Intent intent = new Intent(context, SignSpotCheckActivity.class);
    ...
    protected void initView() {
        ...
        binding.bannerRightText.setOnClickListener(v -> {
            ...
            try {
                ...
                e.printStackTrace();
                ToastUtil.showToast(SignSpotCheckActivity.this, "保存签名失败");
                return;
            }
            switch (name){
                case "执法人员签名":
                    RxBus.getInstance().post("SpotCheckZFRY", filePath);
                    break;
                case "被检查单位负责人签名":
                    RxBus.getInstance().post("SpotCheckDWFZRQM", filePath);
                    break;
                case "见证人签名":
                    RxBus.getInstance().post("SpotCheckJZRQM", filePath);
                    break;
                default:
                    break;
